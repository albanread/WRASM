    ...
    fn to_layout_graph(&self, size_estimator: &dyn SizeEstimator) -> Result<LayoutGraph> {
        ...
        for id in vertex_ids {
            ...
            if let Some(vt) = &vertex.vertex_type {
                ...
                // For @{ shape: name } extension shapes, also surface the bare
                // shape name so renderers can look it up in their own
                // registries without depending on `Debug` formatting.
                if let FlowVertexType::Custom(name) = vt {
                    node.metadata
                        .insert("shape".to_string(), name.clone());
                }
            }
...
fn vertex_type_to_shape(vt: &FlowVertexType) -> NodeShape {
    match vt {
        ...
        FlowVertexType::Custom(_) => NodeShape::Custom,
    }
}
