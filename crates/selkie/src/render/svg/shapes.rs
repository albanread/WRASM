...
use crate::diagrams::flowchart::{FlowVertex, FlowVertexType, LabelAlign};
...
pub fn render_shape(
    ...
    let shape = match shape_type {
        ...
        FlowVertexType::Custom(_) => {
            // Renderer-defined extension shapes have no SVG counterpart in
            // selkie. Fall back to a rectangle so the SVG output still
            // produces a placeholder of the correct size.
    ...
    let (label_x, text_anchor) = match label_align.unwrap_or(LabelAlign::Center) {
        LabelAlign::Left => (x + 12.0, "start"),
        LabelAlign::Center => (cx, "middle"),
        LabelAlign::Right => (x + w - 12.0, "end"),
    };
    ...
    let label = SvgElement::text(label_x, cy, label_text).with_attrs(label_attrs);
...
mod tests {
    ...
    fn test_dark_fill_gets_white_text() {
        ...
        let shape_element = render_shape(&node, &vertex, &theme, Some(style), None);
    ...
    fn test_no_style_no_text_fill_override() {
        ...
        let theme = Theme::default();
        let shape_element = render_shape(&node, &vertex, &theme, None, None);
        let svg = shape_element.to_svg(0);
