...
use crate::diagrams::flowchart::{EdgeAnnotationOverrides, EdgeStroke, FlowEdge, LineStyle};
...
pub fn render_edge_parts(
    ...
    flow_edge: &FlowEdge,
    _theme: &Theme,
    overrides: &EdgeAnnotationOverrides,
) -> EdgeRenderResult {
    let edge_id = &layout_edge.id;
    let route_mode = layout_edge
        .metadata
        .get("annotation_route_mode")
        .map(|value| value.as_str())
        .unwrap_or("curved");

    // Build edge path
    let path = if !layout_edge.bend_points.is_empty() {
        let path_d = match route_mode {
            "manual" | "straight" | "orthogonal" => build_path(&layout_edge.bend_points),
            _ => build_curved_path(&layout_edge.bend_points),
        };
        ...
        if let Some(line_color) = overrides.line_color.as_deref() {
            style_parts.push(format!("stroke: {line_color}"));
        }
        if let Some(line_width) = overrides.line_width {
            attrs = attrs.with_stroke_width(line_width);
            style_parts.push(format!("stroke-width: {line_width}px"));
        }
        match overrides.line_style {
            Some(LineStyle::Solid) => {}
            Some(LineStyle::Dash) => {
                attrs = attrs.with_stroke_dasharray("6,4");
                style_parts.push("stroke-dasharray: 6,4".to_string());
            }
            Some(LineStyle::Dot) => {
                attrs = attrs.with_stroke_dasharray("2,4");
                style_parts.push("stroke-dasharray: 2,4".to_string());
            }
            None => {}
        }
        if !style_parts.is_empty() {
            attrs = attrs.with_style(&style_parts.join(";"));
        }

        // Apply arrow markers
        if let Some(end_arrow) = overrides.end_arrow {
            if let Some(marker_url) = markers::get_override_marker_url(end_arrow, false) {
                attrs = attrs.with_attr("marker-end", &marker_url);
            }
        } else if let Some(marker_url) = markers::get_marker_url(flow_edge.edge_type.as_deref()) {
            attrs = attrs.with_attr("marker-end", &marker_url);
        }
        if let Some(start_arrow) = overrides.start_arrow {
            if let Some(start_marker_url) = markers::get_override_marker_url(start_arrow, true) {
                attrs = attrs.with_attr("marker-start", &start_marker_url);
            }
        } else if let Some(start_marker_url) =
    ...
    let label = if !flow_edge.text.is_empty() {
        if let Some(mut label_pos) = layout_edge.label_position {
            if let Some(offset) = overrides.label_offset {
...
mod tests {
    ...
    fn test_edge_label_renders_text() {
        ...
        let result = render_edge_parts(
            &layout_edge,
            &flow_edge,
            &theme,
            &EdgeAnnotationOverrides::default(),
        );
