...
use crate::diagrams::flowchart::{FlowSubGraph, FlowchartDb, LabelAlign};
...
impl SvgRenderer {
    ...
    pub fn render_flowchart(&self, db: &FlowchartDb, graph: &LayoutGraph) -> Result<String> {
        ...
        doc.set_physical_size_cm(graph_overrides.width_cm, graph_overrides.height_cm);

        // Add theme styles
        if self.config.embed_css {
            let mut css = self.config.theme.generate_css();
            if let Some(annotation_css) = graph_annotation_css(&graph_overrides) {
                css.push_str("\n/* Annotation CSS */\n");
                css.push_str(&annotation_css);
            }
        ...
        if let Some(canvas_fill) = graph_overrides.canvas_fill.as_deref() {
            doc.add_cluster(
                SvgElement::rect(view_min_x, view_min_y, view_width, view_height).with_attrs(
                    Attrs::new()
                        .with_class("annotation-canvas-bg")
                        .with_fill(canvas_fill),
                ),
            );
        }
        ...
        for subgraph in db.subgraphs() {
            if let Some(element) = self.render_subgraph(db, subgraph, graph) {
        ...
        for edge in &graph.edges {
            ...
            let flow_edge = db
                .edges()
                .iter()
                .find(|e| e.id.as_ref().map(|id| id == &edge.id).unwrap_or(false))
                .or_else(|| {
                    db.edges().iter().find(|e| {
                        e.start == edge.sources.first().map(|s| s.as_str()).unwrap_or("")
                            && e.end == edge.targets.first().map(|s| s.as_str()).unwrap_or("")
                    })
                });
            if let Some(flow_edge) = flow_edge {
                let edge_overrides = db.edge_annotation_overrides_for(flow_edge);
                let result =
                    edges::render_edge_parts(edge, flow_edge, &self.config.theme, &edge_overrides);
        ...
        for node in &graph.nodes {
            ...
            if let Some(vertex) = db.vertices().get(&node.id) {
                ...
                let node_overrides = db.node_annotation_overrides(&node.id);
                let styles = combine_style_strings(
                    db.get_compiled_styles(vertex),
                    node_annotation_style(&node_overrides),
                );
                let shape_element = shapes::render_shape(
                    node,
                    vertex,
                    &self.config.theme,
                    styles.as_deref(),
                    node_overrides
                        .label_align
                        .or(graph_overrides.node_label_align),
                );
    ...
    fn calculate_flowchart_bounds(
        ...
        for subgraph in db.subgraphs() {
            if let Some((box_x, box_y, box_width, box_height)) =
                flowchart_subgraph_box(db, subgraph, graph)
            {
                min_x = min_x.min(box_x);
                min_y = min_y.min(box_y);
                max_x = max_x.max(box_x + box_width);
                max_y = max_y.max(box_y + box_height);
            }
        }
    ...
    fn render_subgraph(
        ...
        let (min_x, min_y, width, height) = flowchart_subgraph_box(db, subgraph, graph)?;
        let overrides = db.group_annotation_overrides(&subgraph.id);

        // Create the background rect
        let mut rect_attrs = Attrs::new().with_class("cluster");
        let mut style_parts = Vec::new();
        if let Some(fill) = overrides.fill.as_deref() {
            style_parts.push(format!("fill: {fill}"));
        }
        if let Some(stroke) = overrides.stroke.as_deref() {
            style_parts.push(format!("stroke: {stroke}"));
        }
        if !style_parts.is_empty() {
            rect_attrs = rect_attrs.with_style(&style_parts.join(";"));
        }
        let rect = SvgElement::rect(min_x, min_y, width, height).with_attrs(rect_attrs);

        let label_align = overrides
            .label_align
            .or(db.graph_annotation_overrides().group_label_align)
            .unwrap_or(LabelAlign::Center);
        let (label_x, text_anchor) = match label_align {
            LabelAlign::Left => (min_x + 12.0, "start"),
            LabelAlign::Center => (min_x + width / 2.0, "middle"),
            LabelAlign::Right => (min_x + width - 12.0, "end"),
        };
...
fn flowchart_subgraph_box(
    db: &FlowchartDb,
    subgraph: &FlowSubGraph,
    graph: &LayoutGraph,
) -> Option<(f64, f64, f64, f64)> {
    let overrides = db.group_annotation_overrides(&subgraph.id);
    let has_geometry_override = overrides.x.is_some()
        || overrides.y.is_some()
        || overrides.width.is_some()
    ...
    let box_y = min_y - padding - title_height;
    let box_width = (max_x - min_x) + padding * 2.0;
    let box_height = (max_y - min_y) + padding * 2.0 + title_height;
    Some((box_x, box_y, box_width, box_height))
}

fn combine_style_strings(base: Option<String>, extra: Option<String>) -> Option<String> {
    match (base, extra) {
        (Some(base), Some(extra)) if !base.is_empty() && !extra.is_empty() => {
            Some(format!("{base};{extra}"))
        }
        (Some(base), _) if !base.is_empty() => Some(base),
        (_, Some(extra)) if !extra.is_empty() => Some(extra),
        _ => None,
    }
}

fn node_annotation_style(
    overrides: &crate::diagrams::flowchart::NodeAnnotationOverrides,
) -> Option<String> {
    ...
    if let Some(line_width) = overrides.line_width {
        parts.push(format!("stroke-width: {line_width}px"));
    }
    if parts.is_empty() {
        None
    } else {
...
fn graph_annotation_css(
    overrides: &crate::diagrams::flowchart::GraphAnnotationOverrides,
) -> Option<String> {
    let mut lines = Vec::new();
    if let Some(font_face) = overrides.font_face.as_deref() {
        let sanitized = sanitize_css_literal(font_face);
        if !sanitized.is_empty() {
            lines.push(format!(
                ".mermaid, .mermaid svg {{ font-family: {}; }}",
                sanitized
            ));
        }
    }
    if let Some(font_size) = overrides.node_label_font_size {
        lines.push(format!(
            ".mermaid .node .label {{ font-size: {}px; }}",
            font_size
        ));
    }
    if let Some(font_size) = overrides.group_label_font_size {
        lines.push(format!(
            ".mermaid .cluster-label {{ font-size: {}px; }}",
            font_size
        ));
    }
    if let Some(font_size) = overrides.edge_label_font_size {
        lines.push(format!(
            ".mermaid .edge-label {{ font-size: {}px; }}",
            font_size
        ));
    }
    if lines.is_empty() {
...
fn sanitize_css_literal(value: &str) -> String {
    value
        .chars()
