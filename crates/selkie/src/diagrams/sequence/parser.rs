...
fn process_participant_stmt(
    ...
    let mut metadata_body: Option<String> = None;
    ...
    for inner in pair.into_inner() {
        match inner.as_rule() {
            ...
            Rule::participant_metadata => {
                // Inner is `json_object`; grab its raw atomic text.
                if let Some(json) = inner.into_inner().next() {
                    metadata_body = Some(json.as_str().to_string());
                }
            }
            _ => {}
        }
    ...
    if !name.is_empty() {
        ...
        if let Some(body) = metadata_body {
            for (k, v) in parse_actor_metadata_kv(&body) {
                db.set_actor_property(&name, &k, &v);
            }
        }
    }
...
/// Tiny key:value extractor for `participant Foo@{ ... }` metadata. Strips
/// the outer `{ }`, splits on commas, and trims surrounding quotes from
/// keys and values. Accepts both `{shape: cloud}` and
/// `{"shape": "cloud"}` styles.
fn parse_actor_metadata_kv(body: &str) -> Vec<(String, String)> {
    let inner = body.trim().trim_start_matches('{').trim_end_matches('}');
    let mut out = Vec::new();
    for pair_text in inner.split(',') {
