...
impl SequenceDb {
    ...
    /// Set a free-form property on an existing actor. Returns true if the
    /// actor was found. Used by the parser to surface
    /// `participant Foo@{ ... }` metadata to consumers.
    pub fn set_actor_property(&mut self, name: &str, key: &str, value: &str) -> bool {
        match self.actors.get_mut(name) {
            Some(a) => { a.properties.insert(key.to_string(), value.to_string()); true }
            None => false,
        }
    }
