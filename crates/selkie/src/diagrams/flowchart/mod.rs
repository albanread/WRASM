...
pub use annotations::{
    ArrowHead, ConnectionPoint, EdgeAnnotationOverrides, FlowAnnotation, FlowAnnotationAttr,
    FlowAnnotationTarget, GraphAnnotationOverrides, GroupAnnotationOverrides, LabelAlign,
    LineStyle, NodeAnnotationOverrides,
};
...
/// Parse and rewrite a flowchart into Selkie's canonical Mermaid formatting.
///
/// This is useful for stable diff tests when an input is semantically unchanged but may use
/// non-canonical direction aliases or statement ordering that Selkie normalizes on writeback.
pub fn format(input: &str) -> crate::error::Result<String> {
    parse(input).map(|db| write(&db))
}
